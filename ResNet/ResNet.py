...
class BottleneckBlk(layers.Layer):
    def __init__(self, out_chan, strides = 1, downsample = None, **kwargs):
        ...
        self.conv2 = layers.Conv2D(out_chan, padding='same', kernel_size=3, strides=strides)
        self.bn2 = layers.BatchNormalization()
        self.relu2 = layers.ReLU()

        self.conv3 = layers.Conv2D(out_chan, padding='same', kernel_size=1, strides=strides)
...
class ResNet(keras.Model):
  def __init__(self, blkType, blk_num_list, **kwargs):
    ...
    self.conv2 = self.build_stage(blkType, True, 64, blk_num_list[0], strides = 1, name = 'conv2_x')

    self.conv3 = self.build_stage(blkType, False, 128, blk_num_list[1], strides = 2, name = 'conv3_x')

    self.conv4 = self.build_stage(blkType, False, 256, blk_num_list[2], strides = 2, name = 'conv4_x')

    self.conv5 = self.build_stage(blkType, False, 512, blk_num_list[3], strides = 2, name = 'conv5_x')
  
  def build_stage(self, blkType, isFirstStage, out_chan, blk_num, strides = 1, name = None):
    downsample = None
    if strides != 1 or isFirstStage == False:
      downsample = keras.Sequential([
          layers.Conv2D(out_chan, kernel_size=1, strides=strides),
